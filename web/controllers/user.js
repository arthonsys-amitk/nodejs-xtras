...
web.create_user = (req, res) => {
	...
	if (typeof req.session.user_data == "undefined" || req.session.user_data === true) {
		...
		req.session.temp_user = null;
		res.render('admin/users/add_user', { "user_data": req.session.user_data, "num_queries": qrycount, "resqueries": resqueries, "member_since": req.session.member_since, "hostname": hostname, 'temp_user': req.session.temp_user });
	}
};
...
web.add_user = (req, res) => {
	...
	if (typeof req.session.user_data == "undefined" || req.session.user_data === true) {
		...
		user.get_user_by_phone_no(req.body.phone).then(function (single_user) {
			if (single_user != null) {
				req.session.alert_data = { alert_type: "danger", alert_msg: "Phone number already exist" };
				req.session.flash_msg = { "msg": "Phone number already exist", "type": "danger" };
				res.locals.flashmessages = req.session.alert_data;
				...
				req.session.temp_user = req.body;
				console.log(req.session.temp_user);
				res.render('admin/users/add_user', { "user_data": req.session.user_data, 'temp_user': req.session.temp_user, "num_queries": qrycount, "resqueries": resqueries, "member_since": req.session.member_since, "hostname": hostname });
				req.session.alert_data = { alert_type: "danger", alert_msg: "Phone number already exist" };
				req.session.flash_msg = { "msg": "Phone number already exist", "type": "danger" };
					...
					if (emailresult == null) {
						geocoder.geocode(req.body.address + req.body.city + req.body.state + req.body.country, function (err, result) {
							...
							if (result != [] && result && result.length > 0) {
								...
								req.session.temp_user = req.body;
								req.session.flash_msg = { "msg": "Invalid Address", "type": "danger" };
								req.session.alert_data = { alert_type: "danger", alert_msg: "Invalid Address" };
								res.locals.flashmessages = req.session.alert_data;
								console.log(req.session.temp_user);
								res.render('admin/users/add_user', { "user_data": req.session.user_data, 'temp_user': req.session.temp_user, "num_queries": qrycount, "resqueries": resqueries, "member_since": req.session.member_since, "hostname": hostname });
							}
						...
						req.session.temp_user = req.body;
						console.log(req.session.temp_user);
						req.session.flash_msg = { "msg": "Email already exist", "type": "danger" };
						req.session.alert_data = { alert_type: "danger", alert_msg: "Email already exist" };
						res.locals.flashmessages = req.session.alert_data;
